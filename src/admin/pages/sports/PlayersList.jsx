...
export default function PlayersList() {
    ...
    const filteredPlayers = players.filter(p => {
        ...
        const matchesPos = posFilter === 'All' || p.position.includes(posFilter.toUpperCase());
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-[var(--card)] p-4 rounded-lg shadow-sm border border-[var(--border)]">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <select
                        ...
                        className="block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-[var(--foreground)] bg-[var(--background)] ring-1 ring-inset ring-[var(--border)] focus:ring-2 focus:ring-[var(--ring)] sm:text-sm"
                    >
                                ...
                                <tr key={idx} className="hover:bg-[var(--muted)]/50 transition-colors">
                                    <td className="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                                        <div className="flex items-center">
                                            <div className="h-10 w-10 flex-shrink-0">
                                                <img className="h-10 w-10 rounded-full object-cover border border-[var(--border)]" src={player.image || '/img/picture.avif'} alt="" />
                                            </div>
                                            <div className="ml-4">
                                                <div className="font-medium text-[var(--card-foreground)]">{player.firstName} {player.lastName}</div>
                                    ...
                                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button className="text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors p-1" title="Edit">
